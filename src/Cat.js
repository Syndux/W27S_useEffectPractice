...
const Cat = () => {
  ...
  const [statusChange, setStatusChange] = useState(
    localStorage.getItem("cat-status") | "418"
  );
  ...
  const handleStatusSubmit = (e) => {
    ...
    if (!VALID_STATUS_CODES.includes(Number(newStatus))) {
      alert(
        `Code ${newStatus} might exist, but it is not a proper Cat Status code.`
      );
      setStatusChange(404);
      return;
    }
  ...
  useEffect(() => {
    localStorage.setItem("cat-status", statusChange);
  }, [statusChange]);
  ...
  return (
    ...
    >
      ...
      <div>
        Current Delay Time between color changes: {delayChange / 1000} seconds
      </div>
